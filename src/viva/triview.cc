...
Triview::Triview(const wxString& title)
  : BasicFrame(title)
{
  ...
  Connect(ID_QUALITY_0, ID_QUALITY_4, wxEVT_COMMAND_MENU_SELECTED,
          wxCommandEventHandler(Triview::OnQuality));

  Connect(wxID_EXIT, wxEVT_COMMAND_MENU_SELECTED,
      wxCommandEventHandler(Triview::OnQuit));

  Connect(wxID_OPEN, wxEVT_COMMAND_MENU_SELECTED,
      wxCommandEventHandler(Triview::OnOpen));

  Connect(wxID_ABOUT, wxEVT_COMMAND_MENU_SELECTED,
          wxCommandEventHandler(Triview::OnAbout));

  this->Connect(wxEVT_PAINT, wxPaintEventHandler(Triview::OnPaint));
  this->Connect(wxEVT_MOTION, wxMouseEventHandler(Triview::OnSearchNode));
  this->Connect(wxEVT_LEFT_DOWN, wxMouseEventHandler(Triview::OnLeftDown));
  this->Connect(wxEVT_LEFT_UP, wxMouseEventHandler(Triview::OnLeftUp));
  this->Connect(TupiLayoutUpdated, wxCommandEventHandler(Triview::OnLayoutUpdated));

  Connect (wxEVT_ACTIVATE, wxActivateEventHandler(Triview::OnActivate));
  ...
  Connect(ID_PLAY, ID_PAUSE, wxEVT_COMMAND_TOOL_CLICKED,
      wxCommandEventHandler(Triview::OnThreadManagement));

  Centre();
}

void Triview::start_runner (void)
{
  ...
  runner = new LayoutRunner (this, NULL);//layout);
  runner->Create();
  runner->Run();
}

void Triview::stop_runner (void)
{
...
void Triview::OnActivate(wxActivateEvent& event)
{
  static int flag = 1;
  if (!runner && flag){
    start_runner();
...
void Triview::OnPaint(wxPaintEvent& event)
{
  wxPaintDC dc(this);
...
void Triview::OnLeftUp (wxMouseEvent& event)
{
  // if (selected){
...
void Triview::OnLeftDown (wxMouseEvent& event)
{
  wxPoint screen;
...
void Triview::OnSearchNode (wxMouseEvent& event)
{
  wxPoint screen;
  event.GetPosition (&screen.x, &screen.y);
...
void Triview::OnQuit(wxCommandEvent& WXUNUSED(event))
{
  Close(true);
}

void Triview::OnOpen(wxCommandEvent& WXUNUSED(event))
{
  std::cout << __FUNCTION__ << std::endl;
}

void Triview::OnQuality(wxCommandEvent& event)
{
  return;
...
void Triview::OnThreadManagement(wxCommandEvent& event)
{
  switch (event.GetId()){
  case ID_PLAY: 
  {
...
void Triview::OnClose (wxCloseEvent& WXUNUSED(event))
{
  std::cout << __FUNCTION__ << std::endl;
  if (runner){
    runner->keepRunning = false;
...
void Triview::OnLayoutUpdated (wxCommandEvent& event)
{
